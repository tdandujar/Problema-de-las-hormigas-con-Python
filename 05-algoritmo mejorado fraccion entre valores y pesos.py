...
def matrizValoresPesos(n):

#Igual que antes pero con los pesos siendo divididos por sus valores para poder maximizar
    ...
    for i in range(len(n)+1):
        for j in range(len(matriz)):
            matriz[i][j]=matrizdeValoresPesos[j]
...
matrizDeValoresPesos = matrizValoresPesos(matrizSinTransformar)
...
start_time = time()
(camino, longCamino, peso) = hormigas(matrizDeValoresPesos,matrizDePesos, numHormigas, pesoMochila)
